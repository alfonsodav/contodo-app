...
export class UserPage implements OnInit {
  ...
  ngOnInit() {
    this.auth.user$.subscribe(async (data) => {
      if (!data) {
        await Storage.get({ key: 'userdb' }).then(
  ...
  async updateUser() {
    ...
    await Storage.set({ key: 'userdb', value: JSON.stringify(this.user) });
    await this.createAlert('GRACIAS POR ACTUALIZAR TU INFORMACIÓN', 'succes');
    } catch (error) {
      console.log(error);
      await this.createAlert('OCURRIO UN ERROR AL ACTUALIZAR TU INFORMACIÓN', 'error');
    }
  ...
  async updatePhoto() {
    ...
    this.auth.setPhoto(this.user.pictureB64, this.user).subscribe(data => {
      ...
      if(data.id_Gamer){
        ...
        this.createAlert('OCURRIO UN ERROR AL ACTUALIZAR TU INFORMACIÓN', 'error');
      }
    });
    //console.log(foto);
  }
