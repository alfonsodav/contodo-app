...
export class WellcomePage implements OnInit {
  ...
  dateValue = '';
  constructor(private auth: AuthService, private alertIon: AlertController, private modalController: ModalController) { }

  ngOnInit() {
    this.auth.user$.subscribe(async data => {
      if(!data){
        await Storage.get({key: 'user'}).then(user => this.user = JSON.parse(user.value));
  ...
  async launchAlert(){
    const alert = await this.alertIon.create({
      message: 'Ocurrio un error inesperado al guardar tus datos, por favor verifica y vuelve a intentar',
      buttons: ['Continuar']
    });
    alert.present();
  }
  ...
  async continue(){
    ...
    await Storage.set({key:'id', value: data.id_Gamer});
